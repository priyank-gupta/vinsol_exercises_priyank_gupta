...
class BusinessCenterHours
	...
	def initialize(start_t, end_t)
		...
		@start_t = DateTime.strptime(start_t, "%H%M").to_time.utc
		@end_t = DateTime.strptime(end_t, "%H%M").to_time.utc
	end
	...
	def update(date, start_t, end_t)
		...
		@days_record["updated_days"][date] = [DateTime.strptime(start_t,"%H%M").to_time.utc, DateTime.strptime(end_t,"%H%M").to_time.utc]		#storing in hash with date as key and start time/end time as values
	end
		...
		loop do
			if is_holiday?(date_temp)			# checking if a particular day is holiday
				...
				if (end_t.hour*100 + end_t.min) >= ((start_t.hour*100 + start_t.min) + meet_dur) && end_t > start_t		#checking if the end time of the day is greater than meeting duration and start time of day/start time of meeting
					...
					meet_dur = meet_dur - (a*100 + b).to_i		#claculating the remaining time left for meeting
					date_temp += 1			#move to next day
				end
